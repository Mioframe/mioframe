...
- Minimum verification: run `pnpm verify --only static`, then open a database document, exercise the touched inline or sheet flow, and refresh or reopen the same view to confirm the rendered state still persists correctly. Use focused verify-managed browser coverage where available.
