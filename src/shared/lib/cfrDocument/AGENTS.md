...
Inherits the rules from `src/shared/lib/AGENTS.md`. Applies to `src/shared/lib/cfrDocument` and its descendants until a deeper `AGENTS.md` refines it.
...
- Minimum verification: run `pnpm verify --only type-check`, then verify the touched legacy caller still loads and migrates the relevant document or repo state through focused verify-managed coverage.
