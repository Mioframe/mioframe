...
describe('verifyEntry', () => {
  ...
  it('runs the Material token guard for focused verification and the unit-test lane', () => {
  ...
  it('does not repeat the guard in full or unrelated focused lanes', () => {
    expect(shouldRunMaterialTokenGuard(['--full'])).toBe(false);
    expect(shouldRunMaterialTokenGuard(['--full', '--only', 'unit-tests'])).toBe(false);
