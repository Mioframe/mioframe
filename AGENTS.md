...
- Before non-trivial changes in `src/shared`, service boundaries, CRDT flows, VFS/filesystem code, schema or migration paths, helper semantics, `.project-memory/`, or any scope that may already have memory, start the task through `pnpm memory:task:start --scope <path> --term <keyword>`. This is the mandatory project-memory entrypoint and it runs discovery by exact scope, parent subsystem, related risky boundaries, and task terms.
- Treat project-memory retrieval as token-budgeted guidance, not a prose dump: prefer the compact digest first, rely on trigger-based warnings for dangerous actions, and promote repeated lessons into stronger artifacts instead of growing prose memory indefinitely.
- Repo-local Codex hooks in `.codex/` automatically preload prompt-matched project memory and nudge discovery before risky edits. They do not replace `pnpm memory:task:start` or `pnpm memory:task:finish`, and they should not turn `git commit`, `git push`, or `Stop` into the main surprise enforcement point.
- Use `.project-memory/WORKFLOW.md` as the canonical `memory:task:start -> read memory -> change -> explicit learning decision -> memory:task:finish` procedure when the task can add, promote, merge, archive, or validate project-memory records.
- When a bug fix, review finding, stronger artifact, contradiction, or corrected wrong assumption touches an existing memory scope, update the matching `.project-memory/` entry in the same change and finish through `pnpm memory:task:finish`: refresh evidence, merge duplicates, promote to a stronger artifact, archive with explicit replacement links, or make an explicit keep or covered-by decision during task finish.
...
- Do not treat project-memory review as optional on risky work: the standard exitpoint is `pnpm memory:task:finish`, and staged risky diffs are also rechecked by pre-commit through `pnpm memory:task:review --staged`.
