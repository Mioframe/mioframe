...
- **V3B — Storybook build execution:** local automatic verification reuses one deterministic `storybook-static` prerequisite across selected Storybook behavior and visual proof through the explicit fail-closed `STORYBOOK_STATIC_SKIP_BUILD=1` contract. GitHub implementation verification deliberately does **not** transfer that static output between runners: `verification-browser (storybook-behavior)` and `verification-browser (visual)` are self-contained lanes that both depend only on `autofix`, build the Storybook they need inside their own bounded Playwright execution, and may run in parallel immediately. Application E2E remains independent. The verifier also owns a narrow `STORYBOOK_BUILD_CI_FALLBACK=1` decision for the case where static Storybook build proof is required but neither browser lane will run; PR #212 review requires that fallback invocation to live inside the already-provisioned `verification-static` job rather than a dedicated no-op-capable runner. Standalone focused behavior/visual commands remain self-contained, and PR preview keeps its separate base-specific Storybook build.
