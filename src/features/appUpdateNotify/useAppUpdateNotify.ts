 ...
 * session when available capability has a Manual `available` candidate that
 * has not already been notified this session. Never shown in Automatic mode,
 * and never shown for `ready`, `activating`, or `failed` — only the
 * `available` phase offers something new to schedule. A strictly newer release number
...
export function useAppUpdateNotify(onView: () => void): void {
  const { status, isCapabilityAvailable, mode, candidate } = useAppUpdate();
  const { addSnackbar } = useSnackbar();

  watch(
    [status, isCapabilityAvailable, mode, candidate],
    ([currentStatus, capabilityAvailable, currentMode, currentCandidate]) => {
      if (currentStatus !== 'update-available') return;
