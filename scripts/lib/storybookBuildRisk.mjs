 ...
 */
export function isStorybookBuildRelevantFile(filePath) {
 ...
 * @param [options.packageJsonOldRef] Git ref to compare the current `package.json` against,
 * for the version-only impact refinement. Pass `null` when no reliable base ref is known;
 * that fails closed to runtime-relevant (full lane).
 * @returns Plan with `mode` and human-readable `reasons`.
 */
export function resolveStorybookBuildPlan(changedFiles, { packageJsonOldRef = null } = {}) {
