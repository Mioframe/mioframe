...
description: 'Use for an independent read-only contract-gate or correction-final review of one bounded Material correction unit. Reviews selected concern evidence and affected owners only; complete PR merge readiness belongs to material-pr-review.'
---
...
- `correction-final` after implementation and conditional adoption for that unit.
...
- classifications, dependencies, owner, correction priority, and proof lane follow the canonical workflow;
...
Mechanical fixes that do not change target, ownership, correction scope, proof decision, or observable behavior do not require another full review. They receive a local orchestrator consistency check.
