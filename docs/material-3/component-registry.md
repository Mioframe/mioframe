...
- Switch: `MDSwitch` is `partial`, first implementation.
  ...
  Token status: `MDSwitch` now defines the official no-icon `--md-comp-switch-*` layer for track width/height/shape/outline, selected and unselected track colors, selected/unselected/pressed handle sizes, selected/unselected/hover/focus/pressed handle colors, disabled selected/unselected handle colors and opacities, disabled track colors/opacity, 40dp state-layer size/shape, and selected/unselected hover/focus/pressed state-layer colors and opacities. Internal derived geometry such as active handle center/outline suppression remains private because the spec does not publish selected-outline or handle-position tokens.
  ...
  Interaction contract status: the focusable host (a `<label>` with `role="switch"`, `aria-checked`, and `aria-disabled`) owns click/Enter/Space activation and intentionally prevents native label-to-input click forwarding, driving the bound `v-model` checkbox purely as a hidden state mirror. This deliberately mirrors `MDCheckbox`'s existing interactive contract while matching the refreshed Switch accessibility docs that require both Space and Enter toggles. The interactive host keeps the 52x32 visual track, a 40dp circular state layer centered on the active handle, and an internal 48dp minimum target layer without growing the visual track height. Presentation mode renders a non-interactive `aria-hidden` div with an explicit `md-switch_presentation` modifier (`pointer-events: none`, `cursor: default`), no input, and no state layer/ripple, so row-level consumers (e.g. `SettingsSwitchListItem`) continue to own the action themselves.
  ...
  Visual or browser verification status: `MDSwitch.test.ts` covers ARIA/role/disabled/presentation/keyboard/pointer/focus/autofocus and target-layer presence contracts; `SettingsSwitchListItem.test.ts` keeps the row-owned presentation-switch contract on the VTU mount path; `tests/e2e/visual/shared-ui.spec.ts` covers switch state galleries, interaction-state galleries, and a 48dp target-layer browser assertion through Storybook.
  Deviations or unsupported features: the current shared primitive implements only the no-icon configuration from the official component family; optional selected/unselected handle icons remain unsupported. Drag gestures described in the accessibility guidance are not implemented as a separate interaction contract. Switch remains `partial`, not `aligned`, because those official configurations are still unsupported.
