...
- Minimum verification: run `pnpm verify --only static`, then change sort direction or sort order in a view, confirm the row order updates, and refresh or reopen the same view to confirm the sorting persisted. Use focused verify-managed coverage where available.
