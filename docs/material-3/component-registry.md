...
- Lists: `MDList`, `MDListItem`, and `MDListContainer` are `partial`.
  ...
  Token status: list surfaces now use list-family CSS variables owned by `MDList` and `MDListItem`, with sizing derived from Material list style and density instead of consumer radius patches.
  Shape status: `MDList` owns `standard` and `segmented` list styles. Standard baseline remains square-edged; segmented expressive owns grouped shape, 2dp gaps, and first/middle/last rounding in the shared primitive instead of consumer overrides.
  Public API status: `MDList` exposes `listStyle: 'standard' | 'segmented'`, `density: 'baseline' | 'expressive'`, and list semantics via `div[role=list]` by default or `ul` when requested. `MDListItem` supports `static`, `single-action`, and `multi-action` modes plus Material anatomy slots `leading`, `overline`, `supportingText`, `trailing`, and `trailingAction`. `single-action` requires either an `@action` listener or an `href`; `multi-action` requires both a primary action and a `#trailingAction` slot.
  ...
  Selection support: full list-level single-select and multi-select semantics are deferred. The component family does not claim supported listbox/option behavior in Storybook or docs for this pass.
  Storybook status: `Material 3/Components/Lists/MDListItem` now documents baseline standard, expressive segmented, DOM contract, interaction-state coverage, and trailing-action geometry using only valid Material configurations.
  Visual or browser verification status: visual stories cover configurations, state geometry, and trailing actions; browser tests cover DOM roles, no nested native actions, segmented wrapper rounding, and trailing-action 48dp targets.
  Deviations or unsupported features: full selection semantics, selection-specific keyboard behavior, and expandable/swipe list variants remain unsupported. Lists remain `partial`.
