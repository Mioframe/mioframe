...
- selected rows use `secondary-container` with `on-secondary-container`, segmented item container color is `surface`, overline typography is `label-small`, and dragged rows render a filled elevated preview using `tertiary-container` / `on-tertiary-container` — no `md.comp.list.list-item.dragged.container.color` token is published, so this is a private List-family implementation decision (see [Dragged state](../../src/shared/ui/Lists/README.md#dragged-state) in the List README), not a documented Material component token;
