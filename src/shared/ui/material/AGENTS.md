...
This file contains only local routing and ownership constraints. Detailed source, architecture, token, testing, review, and completion policy lives in `docs/material-3` and the applicable Material skill.

Do not duplicate those documents here.

## Generalization boundary

Shared instructions contain only artifact-independent rules. Concrete selectors, DOM nodes, custom-property names, token values, state endpoints, defects, and proposed structures belong in the owning README, AUDIT, implementation, tests, fixtures/stories, roadmap, or task-specific PR description.

A pilot finding may change shared instructions only after it is expressed as a rule applicable to every artifact owning the same risk.

## Routing

Use `material <artifact-or-request>` by default.

- component family → `material-component` → `material-component-authoring`;
...
README persists source limitations, implementation state, known defects, consumers, verification, and explicit operator feedback. A reported visible defect remains rejected until production behavior changes and the user explicitly accepts the replacement.
...
- Follow the repository-wide rule against unnecessary DOM nodes. Ownership maps describe responsibilities, not a required element count; one existing element may own several compatible roles.
...
Implementation finishes only when code and README agree, source limitations and remaining gaps are explicit, consumers and obsolete ownership are handled, applicable proof exists, and local verification passes.
