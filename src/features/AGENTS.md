...
- Minimum verification: run `pnpm verify --only static`, then open the touched action, exercise cancel and submit paths, and reopen it to confirm state reset or persistence behavior. Use focused verify-managed unit or browser checks for the changed flow.
