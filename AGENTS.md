...
- When a new assertion-bearing automated test/spec is required, or an existing one materially changes its oracle/expectations/assertions/failure semantics, use `test-authoring` in a separate test-author context before production implementation. Intentional visual-baseline changes also use `test-authoring` with `visual-regression-testing`; when new pixels can exist only after implementation, the implementation context must not create or approve that baseline. Test authorship never changes proof ownership/type/placement.
