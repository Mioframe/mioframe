...
- Lists: `MDList`, `MDListItem`, and `MDListContainer` are `partial`.
  ...
  Visual or browser verification status: visual stories cover configurations, state geometry, trailing actions, and selection modes; browser tests cover DOM roles, no nested native actions, segmented wrapper rounding, independent trailing actions, selected indicators, and trailing-action 48dp targets.
  Deviations or unsupported features: live Figma node verification was blocked by the current Figma MCP plan limit during this pass, selection rows currently use a shared checkmark indicator instead of Material-specific radio or checkbox controls, and expandable/swipe list variants remain unsupported. Lists remain `partial`.
