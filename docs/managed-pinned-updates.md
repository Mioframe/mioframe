...
The marker and controller record are independent guards rather than a cross-storage transaction. Valid state is authoritative even if marker persistence was interrupted; the next install repairs the marker without changing release selection. Marker presence with missing state is evidence of state loss and never authorizes bootstrap.
