...
Use this skill before non-trivial implementation, workflow, shared UI, storage, diagnostics, Material, performance-sensitive, or cross-layer changes unless an applicable workspace policy defines a deterministic authoring path that resolves every required decision from authoritative sources.

The handoff is the implementation contract and must not be silently replaced during coding or review.

## When to use

Use this skill when the task changes or clarifies any of these and no deterministic workspace path fully resolves them:
...
- Do not expand stable workspace rules, FSD explanations, Material rules, or testing policy unless directly relevant.
...
- Goal
- Confirmed current behavior and workspace evidence, or confirmed product requirement and source
- Non-goals
...
- Implementation readiness:
  ...
  - dependencies, inputs, and worker-access boundaries explicit
...
Considering a risk does not require supporting it. Include behavior or non-functional guarantees only when required by a scenario, existing consumer, workspace invariant, platform constraint, data-safety rule, or measured need.
...
- required dependencies, inputs, or worker-access boundaries are unclear or unavailable;
