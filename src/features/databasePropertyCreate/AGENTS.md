...
- Minimum verification: run `pnpm verify --only type-check`, then create a property of the touched kind, confirm it appears in the current view and related editors, and reopen the dialog to confirm the draft reset. Use focused verify-managed coverage where available.
