...
- Entity UI may render domain state and emit intents, but user mutation flows, confirmation logic, platform prompts, and snackbar orchestration belong in `features` or higher composition.
...
- Do not move dialog flows, action menus, submit orchestration, or platform prompt orchestration into `entities`.
...
- Do not expose provider capabilities, service clients, or mixed read/write models through ordinary entity display records.
...
- Minimum verification: run `pnpm verify --only static`, then exercise the touched entity through at least one consuming feature or widget and confirm loading, display, and emitted intents still match the contract. Use focused verify-managed tests for changed derivation or wiring.
