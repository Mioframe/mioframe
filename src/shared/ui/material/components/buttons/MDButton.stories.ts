    ...
    docs: {
      description: {
        component: [
          ...
          '',
          '**Loading (Mioframe extension, not a published Material token)**: `loading` accepts `boolean | number`. `true` renders an indeterminate `MDCircularProgressIndicator`; a finite number that clamps into `(0, 1]` renders real determinate progress (values above `1` clamp to `1`). A numeric value that clamps to `0` — `0` itself, negative values, `NaN`, `Infinity`, or `-Infinity` — is still an active loading state (not the same as `false`) but renders the same indeterminate visual as `true`: `MDCircularProgressIndicator` has no distinct ring for a static zero fill, so `MDButton` forwards `undefined` rather than a fake determinate `0`. Any negative, above-range, or non-finite numeric input logs a development warning; plain `0` does not warn, since it is a valid input with defined (indeterminate) rendering. The button stays enabled and clickable while loading — the original label remains the accessible name, and the button exposes `aria-busy="true"` while loading (omitted otherwise); the rendered progress indicator carries `aria-hidden="true"` since it is decorative. The indicator color routes through the same rendered label color and opacity as the label it visually replaces, so a disabled loading button dims the indicator to the same effective content opacity.',
        ].join('\n'),
      },
    },
  },
