...
- Cards: `MDCard` is `partial`.
  ...
  Interaction contract status: actionable cards do not support nested buttons/links as a documented pattern (Material's accessibility guidance avoids stacking actionable surfaces); this is documented in `shared/ui/Card/README.md` and Storybook notes rather than enforced with a DOM scan of the (arbitrary) default slot. Disabled `button` cards use native `disabled`; disabled `link` cards use `aria-disabled="true"` + `tabindex="-1"` plus a script-level click guard, since links ignore native `disabled`.
  Storybook status: `shared/ui/MDCard` documents variants, a static card with internal actions, actionable button/link cards, disabled actionable cards, a dragged card, a forced-interaction-state gallery (`MDStateLayerForcedStateProvider`) per variant, and a visual state gallery (static/actionable/disabled/dragged per variant).
  Visual or browser verification status: `MDCard.test.ts` covers default variant/mode, per-variant modifier classes, static non-actionable behavior (no role/tabindex/state layer, no `action` emit), button mode (native `button`, `type`, `action` emit, disabled), link mode (native anchor, `href`, `action` emit, disabled aria-disabled/tabindex), and state-layer presence gated by mode. `tests/e2e/visual/shared-ui.spec.ts` covers the visual state gallery, the forced interaction-state gallery, static-card non-actionable DOM contract, actionable button-card click/keyboard activation, and disabled link-card blocked-navigation semantics.
  Deviations or unsupported features: no icon/media anatomy, no `container.surface-tint-layer.color` tonal elevation overlay, and no support for nested actionable content inside an actionable card (documented, not enforced by a DOM scan). Internal padding/gap (16dp padding, 8dp content gap) is a project layout default, not an official Material token.
