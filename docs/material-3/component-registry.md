...
- Switch: `MDSwitch` is `partial`, first implementation.
  Material docs checked: `components/switch/specs.md`, `components/switch/guidelines.md`, and `components/switch/accessibility.md`, refreshed from the official Material cache on 2026-06-30.
  Token status: `MDSwitch` defines the official no-icon `--md-comp-switch-*` layer for track width/height/shape/outline, selected and unselected track colors, selected/unselected/pressed handle sizes, selected/unselected/hover/focus/pressed handle colors, disabled selected/unselected handle colors and opacities, disabled track colors/opacity, 40dp state-layer size/shape, and selected/unselected hover/focus/pressed state-layer colors and opacities. The resolved interaction color is mapped into `MDStateLayer` through the generic `--md-private-state-layer-color` contract; `MDStateLayer` does not read switch-specific variables directly. Internal derived geometry such as active handle center and selected-track outline suppression remains private because the spec does not publish selected-outline or handle-position tokens.
  ...
  Interaction contract status: the focusable host (a `<label>` with `role="switch"`, `aria-checked`, and `aria-disabled`) owns click/Enter/Space activation and intentionally prevents native label-to-input click forwarding, driving the bound `v-model` checkbox purely as a hidden state mirror. This deliberately mirrors `MDCheckbox`'s existing interactive contract while matching the refreshed Switch accessibility docs that require both Space and Enter toggles. The interactive host keeps the 52x32 visual track, a 40dp circular state layer centered on the active handle, stacked above the track and below the handle, and an internal 48dp minimum target layer without growing the visual track height. Focus indication is intentionally delegated to the project's global focus-indicator system rather than reimplemented inside `MDSwitch`. Presentation mode renders a non-interactive `aria-hidden` div with an explicit `md-switch_presentation` modifier (`pointer-events: none`, `cursor: default`), no input, and no state layer/ripple, so row-level consumers (e.g. `SettingsSwitchListItem`) continue to own the action themselves.
  ...
  Visual or browser verification status: `MDSwitch.test.ts` covers ARIA/role/disabled/presentation/keyboard/pointer/focus/autofocus, target-layer presence, and the generic state-layer color variable contract; `SettingsSwitchListItem.test.ts` keeps the row-owned presentation-switch contract on the VTU mount path; `tests/e2e/visual/shared-ui.spec.ts` covers switch state galleries, interaction-state galleries, and a 48dp target-layer browser assertion through Storybook.
  Deviations or unsupported features: the current shared primitive implements only the no-icon configuration from the official component family; optional selected/unselected handle icons remain unsupported. Focus-indicator tokens are documented by Material, but this project intentionally delegates focus rendering to the global focus-indicator system instead of drawing a local switch ring. Official handle elevation and handle shadow-color tokens are known but remain unsupported until the shared elevation/shadow token pipeline can represent them without a switch-specific one-off shadow. Drag gestures described in the accessibility guidance are not implemented as a separate interaction contract. Switch remains `partial`, not `aligned`, because those official configurations and token gaps are still unsupported.
