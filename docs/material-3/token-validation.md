...
`scripts/materialStaticValidation.mjs` (the `material-static` verify step) implements only a minimal filesystem baseline: exact canonical placement of new official components (diff-aware; an explicitly supplied `--base-ref` that git cannot read fails the run rather than silently skipping this check; a malformed invocation — `--base-ref` with no value, an empty value, or immediately followed by another option — is rejected as a CLI usage error), and empty canonical directories (including the namespace roots themselves), placeholder files (including governance files), and an empty root barrel. `.oxlintrc.json` enforces static dependency-direction boundaries by alias, and `eslint.config.mjs` enforces the same alias boundaries for dynamic `import()` calls (Material must not import product layers; generic `shared/lib` must not depend on Material; external consumers, including product-layer files, must use the Material public API) through `no-restricted-imports` and a dedicated `local/no-restricted-dynamic-imports` rule respectively, using anchored matching that blocks only actual product-layer roots/descendants. Relative-path layer imports are not resolved and are not automatically enforced; they remain a review-driven concern, to avoid a speculative path resolver or a false-positive-prone regex policy. Proven by tests that execute the real lint path — including the real merged ESLint configuration — against fixtures rather than inspecting configuration JSON or an extracted rule value.
