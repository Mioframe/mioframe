...
- Minimum verification: run `pnpm verify --only static`, then exercise the affected widget in its host page and confirm the touched action or read state still updates in place. Use focused verify-managed browser coverage when the observable behavior depends on the DOM.
