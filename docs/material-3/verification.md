...
This runs a minimal, deterministic filesystem validator (`scripts/materialStaticValidation.mjs`) over `src/shared/ui/material`. It is a filesystem-only baseline, not a complete architecture framework: new official Material components must be created under the exact canonical `components/<family>/MD*.vue` path (diff-aware, grandfathers pre-existing legacy files, and stays disabled only when no comparison base ref is supplied at all — an explicitly supplied `--base-ref` that git cannot read fails the run instead of silently skipping this check, and a malformed invocation — `--base-ref` with no value, an empty value, or immediately followed by another option — is rejected as a concise CLI usage error before any check runs), and every tracked file under the Material root, recursively at every depth including the root itself, is checked for placeholder content — empty or whitespace-only files and `.gitkeep` files (including governance files such as `README.md`) are rejected. Git does not track empty directories, so directory emptiness is not validated. It does not parse imports, exports, CSS, or Vue/TypeScript source.

Dependency-direction and public-import boundaries (Material must not import product layers; generic `shared/lib` must not depend on Material; external consumers, including product-layer files, must use the Material public API instead of a deep import) are **not** automated, statically or dynamically, by `.oxlintrc.json`, `eslint.config.mjs`, or any other mechanism. They remain review-driven, intentionally, until real component migrations establish the actual public entry-point structure, whether family-level entry points are needed, the migration and compatibility-export model, and whether a general resolver-aware FSD boundary mechanism is justified.

It is a cheap, blocking check that runs in every `pnpm verify` invocation — focused and full/release — right after formatting and linting, before type-checking and any browser/visual/mutation check. It never claims to verify semantic Material correctness, visual equivalence, blueprint meaning, source interpretation, architecture profiles, production-file completeness, CSS layer order, test-artifact/story identity, dependency-direction, or migration-residue completeness; see `token-validation.md` for the complete rule catalogue, validation-class model, and which checks remain review-driven until real migrations justify automating them.
