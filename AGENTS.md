...
- When a new assertion-bearing automated test/spec or accepted visual baseline is required, or an existing one materially changes its oracle/expectations/assertions/failure semantics, use `test-authoring` in a separate test-author context before production implementation. This changes authorship only, not proof ownership/type/placement; accepted proof remains read-only to the implementation context until the first GREEN result.
