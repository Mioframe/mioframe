...
- a value outside `24..240` is clamped to the nearest bound; a non-finite value falls back to the default `48`; in both cases a development-mode warning is emitted (deterministic, not a silent forward);
- arbitrary CSS strings, percentages, `calc()`, and m3e variable names are not public API.

Normalization and clamping are implemented and tested locally in `MDLoadingIndicator.vue` / `MDLoadingIndicator.test.ts`.
...
| Scalable size (`loading-indicator/overview`, specs, "Responsive layout")               | yes — required as public numeric `size` API and Button mapping | `size?: number`, dp mapped to px, default `48`, clamped to `24..240`                                                   | `--m3e-loading-indicator-active-indicator-size` documented; 2.6.2 implementation reads `--m3e-loading-indicator-size`, `divergent`               | `MDLoadingIndicator` | `temporary-renderer-workaround` — map `size` to the confirmed effective variable name, not the documented one; recorded above with removal trigger                                           | `MDLoadingIndicator.test.ts` (default, explicit, lower-bound clamp, upper-bound clamp, exact CSS-variable mapping)                       |
...
- colocated component-contract tests (`MDLoadingIndicator.test.ts`) for the accessible label, default size, explicit valid size, lower-bound clamp, upper-bound clamp, and the exact private CSS-variable mapping;
- colocated stories (`MDLoadingIndicator.stories.ts`) for default presentation, an independent size matrix (`24`/`32`/`40`/default `48`), and inherited color on colored surfaces — standalone visual proof, not only through `MDButton`;
...
- the demand-scoped Material Vue API (`label`, numeric `size`) is implemented and validated;
...
- tests, stories, standalone visual proof, and focused type-check pass.
