...
- Minimum verification: run `pnpm verify --only type-check`, then switch between the touched views, confirm the active chip and layout update correctly, and refresh or reopen the document to confirm selection persistence. Use focused verify-managed browser coverage where available. Final completion uses the single task-scope gate defined by the root `AGENTS.md`; this nested minimum does not add another command boundary.
