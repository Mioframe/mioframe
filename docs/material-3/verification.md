...
This runs a minimal, deterministic filesystem validator (`scripts/materialStaticValidation.mjs`) over `src/shared/ui/material`. It is a filesystem-only baseline, not a complete architecture framework: new official Material components must be created under the exact canonical `components/<family>/MD*.vue` path (diff-aware, grandfathers pre-existing legacy files, and stays disabled only when no comparison base ref is supplied at all — an explicitly supplied `--base-ref` that git cannot read fails the run instead of silently skipping this check), and empty canonical directories (including the `components`/`foundation`/`patterns` namespace roots themselves), empty or whitespace-only placeholder files (including governance files such as `README.md`), and an empty root barrel are rejected. It does not parse imports, exports, CSS, or Vue/TypeScript source.

Dependency-direction rules (Material must not import product layers, by alias or by relative path, statically or dynamically; generic `shared/lib` must not depend on Material, statically or dynamically; external consumers, including product-layer files, must use the Material public API instead of a deep import, statically or dynamically) are enforced through ESLint/oxlint `no-restricted-imports` overrides in `.oxlintrc.json` for static imports/exports, and through real ESLint's `no-restricted-syntax` (targeting `ImportExpression`) in `eslint.config.mjs` for dynamic `import()` calls, since oxlint has no dynamic-import-aware rule. Relative-path and root-alias matching is anchored so it blocks only actual product-layer roots and descendants, not unrelated paths that merely contain a layer name. These are part of the existing `oxlint`/`eslint` verify steps, not this validator, and are proven by tests that execute the real lint path against fixtures.
