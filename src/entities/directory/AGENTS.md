...
- Minimum verification: run `pnpm verify --only type-check`, then enter the touched directory view, trigger a rename, create, or remove flow through the existing features, and confirm the listing updates without losing navigation state. Use focused verify-managed browser coverage when available. Final completion uses the single task-scope gate defined by the root `AGENTS.md`; this nested minimum does not add another command boundary.
