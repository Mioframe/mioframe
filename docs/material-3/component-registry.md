...
- Lists: `MDList`, `MDListItem`, and `MDListContainer` are `partial`.
  ...
  DOM structure status: within `MDList`, non-selectable lists render list semantics on the container and stable listitem wrappers on each item. Single-action and multi-action rows render internal action elements inside the wrapper, so the final list contract is no longer `button[role=listitem]` or `a[role=listitem]`. Selection lists render `role="listbox"` with `role="option"` items, disabled-aware roving tab stops, and selected-state indicators that do not rely on color alone. Rows missing a selection `value` render as disabled options instead of invalid `listbox > listitem` DOM. Secondary controls remain outside the primary action surface.
  Size status: baseline uses 56dp / 72dp / 88dp minimum row heights; expressive uses a 64dp one-line minimum and keeps the 72dp / 88dp multi-line thresholds. Shared consumers such as menus can override the minimum row height through `--md-comp-list-item-min-container-height` without depending on item internals.
  ...
  Storybook status: `Material 3/Components/Lists/MDListItem` now documents baseline standard, expressive segmented, interaction states, trailing-action geometry, DOM contract, and selection modes using Material terminology.
  Visual or browser verification status: visual stories cover configurations, state geometry, trailing actions, and selection modes; browser tests cover DOM roles, no nested native actions, segmented wrapper rounding, independent trailing actions, disabled-aware listbox navigation, selected indicators, and trailing-action 48dp targets.
