...
Declare component-token defaults on the family's stable `.md-<component>` block selector. Do not put component-token defaults on `:root`; `:root`/global ownership belongs to foundation/reference/system theme concerns, while component tokens belong to the component-family styling boundary.
...
### Public token catalogue is component-scoped executable CSS
...
Why: the public contract is executable CSS, scoped to the component-library block class, preserves official aliases where they exist, and preserves an official literal when Material does not provide a system alias.
...
Why: a component-owned default is unnecessarily promoted to global root ownership.
...
7. Verify component defaults are declared under the stable family block class rather than global `:root` ownership.
...
- Declaring component-token defaults globally on `:root` instead of the stable component block class.
