...
| `--md-comp-list-list-item-between-space`                                                             | `md.comp.list.list-item.between-space`                                    | Symmetric inline content padding (edge ↔ leading/trailing content); defaults to `12dp` |
...
| `--md-comp-list-list-item-leading-space`                                                             | `md.comp.list.list-item.leading-space`                                    | Gap between leading element and content; defaults to `16dp`                            |
...
`between-space` is one symmetric token applied to both inline-start and inline-end content padding (the gap from the row edge to the leading/trailing content column). `leading-space` and `trailing-space` are the gaps between the leading/trailing element and the content column itself. `top-space`/`bottom-space` are the block padding above/below the content column. Values (`between-space` `12dp`, `leading-space` `16dp`, `trailing-space` `16dp`, `top-space`/`bottom-space` `10dp` each) are taken verbatim from the literal `md.comp.list.list-item.*` List Common spec token table, confirmed through the `material3` MCP server's cached `m3.material.io` snapshot. A previous pass had `between-space` and `leading-space` swapped (`16dp`/`12dp`); this pass corrected them against the literal token table instead of the MCP page's interactive widget rendering.
