...
invocationSource =
  invocationSource.slice(0, sectionStart) +
  canonicalSection +
  invocationSource.slice(sectionEnd);
...
const oldOwnership = '`scripts/lib/changedPaths.mjs` now owns a repository-wide, status-aware changed-path model (see Phase 1, step 1, below). Local, local-base, and GitHub Actions planning all use NUL-delimited `git diff --name-status` output, preserve deleted paths, and expose both sides of a rename. `--files` remains an explicit existing-path override handled separately from Git diff planning.';
const newOwnership = '`scripts/lib/verifyInvocation.mjs` owns effective invocation precedence and resolves full, explicit-files, GitHub-base, local-base, and local scopes. For non-full scopes, `scripts/lib/changedPaths.mjs` owns the repository-wide, status-aware Git changed-path model (see Phase 1, step 1, below). Git-backed planning uses NUL-delimited `git diff --name-status` output, preserves deleted paths, and exposes both sides of a rename. `--files` remains an explicit existing-path override handled separately from Git diff planning.';
