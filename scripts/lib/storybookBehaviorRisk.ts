...
/** One explicit registry mapping of source path prefixes to Storybook behavior specs. */
export interface StorybookBehaviorScenarioScope {
  name: string;
 ...
 */
export const STORYBOOK_BEHAVIOR_STANDALONE_SPECS: string[] = [];
 ...
 */
export const STORYBOOK_BEHAVIOR_SCENARIO_SCOPES: StorybookBehaviorScenarioScope[] = [
  {
 ...
 */
export function findStorybookBehaviorSpecFiles(dir: string): string[] {
 ...
 */
export function findColocatedBrowserSpecFiles(
  dir: string = COLOCATED_BROWSER_SPEC_ROOT_DIR,
): string[] {
  return findFilesRecursive(dir, COLOCATED_BROWSER_SPEC_SUFFIX);
}

function getAllRegistrySpecs(scenarios: readonly StorybookBehaviorScenarioScope[]): string[] {
  return uniqSorted(scenarios.flatMap((scenario) => scenario.specs));
}

function getScenariosForPath(filePath: string): StorybookBehaviorScenarioScope[] {
 ...
 */
export function isStorybookBehaviorSpecPath(filePath: string): boolean {
 ...
 */
export function isColocatedBrowserSpecPath(filePath: string): boolean {
  return (
 ...
 */
function getColocatedBrowserSpecOwnerRoot(specPath: string): string {
 ...
 */
function findColocatedBrowserSpecOwners(
  filePath: string,
 ...
 */
export function isStorybookBehaviorSupportPath(filePath: string): boolean {
  return (
...
/** Validation result for the Storybook behavior scenario registry. */
export interface StorybookBehaviorRegistryValidation {
  valid: boolean;
  errors: string[];
}

/** Test-only overrides for {@link validateStorybookBehaviorScenarioRegistry}. */
export interface ValidateStorybookBehaviorScenarioRegistryOverrides {
  scenarios?: StorybookBehaviorScenarioScope[];
  standaloneSpecs?: string[];
  specDir?: string;
  /**
   * Test-only override for the discovered spec paths, bypassing filesystem
   * discovery under `specDir`. Lets registry coverage be tested against
   * deterministic virtual paths.
   */
  specFiles?: string[];
  /**
   * Test-only override for the spec-existence check used for registry and
   * standalone entries, bypassing the real filesystem.
   */
 ...
 */
export function validateStorybookBehaviorScenarioRegistry(
  overrides: ValidateStorybookBehaviorScenarioRegistryOverrides = {},
): StorybookBehaviorRegistryValidation {
  ...
  for (const spec of specFiles) {
    if (!coveredSpecs.has(spec)) {
      errors.push(
        `Storybook behavior spec ${spec} is not covered by STORYBOOK_BEHAVIOR_SCENARIO_SCOPES or STORYBOOK_BEHAVIOR_STANDALONE_SPECS in scripts/lib/storybookBehaviorRisk.ts`,
      );
    }
  }
...
export interface ResolveStorybookBehaviorPlanOptions {
  ...
  fileExists?: (filePath: string) => boolean;
  /**
   * Test-only override for the discovered owner-local `*.browser.spec.ts`
