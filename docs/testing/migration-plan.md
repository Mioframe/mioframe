...
- `md-button-legacy-surface-linux.png` was intentionally reaccepted solely because the Button-owned `LegacySurfaceColorOwnership` story removed an unrelated standalone `MDLoadingIndicator` fixture that was never part of the Button visual contract; the remaining pixels, including Button's own `Surface loading` composition, are unchanged;
