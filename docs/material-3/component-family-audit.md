...
- visual-lane browser coverage lives in `tests/e2e/visual/shared-ui/md-button-family.spec.ts` (split out of the former `shared-ui.spec.ts` monolith, which no longer contains any Button-family test or helper): every story tagged `visual` has an explicit `toHaveScreenshot()` assertion, including the toggle-shapes matrix and the toggle-interaction matrix, which previously carried the `visual` tag with no screenshot; the interaction-states screenshot now shows hover/focus/pressed for all five color styles (elevated/filled/tonal/outlined/text), not a representative subset;
- a table-driven token-routing test covers all five color styles for hover/focus/pressed label, icon, state-layer color, state-layer opacity, and (where the style defines one) per-state elevation, plus selected/unselected toggle routing; every property is asserted against the exact literal the story fixture set for that property (each property uses an independent fixture value), not only that two rendered values differ from each other; a companion default-role test renders each style with no override and confirms the resting container/label/icon color resolves to its documented `--md-sys-color-*` role (`filled`→`primary`/`on-primary`, `tonal`→`secondary-container`/`on-secondary-container`, `outlined`/`text`→`on-surface-variant`/`primary`, `elevated`→`surface-container-low`/`primary`), plus a second default-role test confirming hover/focus/pressed state-layer opacity resolves to the documented `--md-sys-state-{hover,focus,pressed}-state-layer-opacity` role for all five styles — alignment is no longer evidenced only by custom override colors, and override routing is verified separately from default (no-override) role resolution;
- state-layer color routing is verified against `--md-private-state-layer-color`, the generic contract variable `MDButton` hands off to `MDStateLayer`, rather than `MDStateLayer`'s own rendered background color; `MDStateLayer`'s CSS has a confirmed defect where its `:global(<state-class>) > &` forced-state selector (used by Storybook fixtures that apply a state class to a component's root instead of driving it through props) compiles without the `> &` combinator, so a forced hover/focus/pressed/dragged/disabled class on a parent never actually paints a visible state-layer tint — visible in existing "interaction states" baselines, which show no state-layer shading despite the forced class. This is generic `MDStateLayer` behavior, not Button-family-owned, real user interaction is unaffected (it drives `MDStateLayer` through props, a different code path), and it is out of scope for this PR; routing tests were designed around it by asserting the contract variable instead of the downstream paint;
