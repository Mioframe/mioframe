...
When standalone implementation requires a new assertion-bearing automated test/spec or accepted visual baseline, or materially changes an existing oracle/expectation/assertion/failure semantic, author that proof first in a separate test-author context following root `test-authoring` and the selected proof-type skill. Required RED evidence is produced there before standalone production implementation. The implementation context treats the accepted proof files as read-only until the first GREEN result and routes a genuine proof/contract conflict back to the test-author or architect instead of rewriting proof to fit m3e/runtime behavior. This changes test authorship only; it does not alter Material owner order, contract sequencing, proof ownership, or workflow state.
