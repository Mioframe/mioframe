...
description: Use only when the Material orchestrator delegates one bounded official target concern set before current implementation is inspected.
...
Execute the preloaded `material-canonical-target` role for the delegated concern set. Return its required result only.
