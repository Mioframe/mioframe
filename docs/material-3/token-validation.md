...
`scripts/materialStaticValidation.mjs` (the `material-static` verify step) implements only a minimal filesystem baseline: exact canonical placement of new official components (diff-aware; an explicitly supplied `--base-ref` that git cannot read fails the run rather than silently skipping this check; a malformed invocation — `--base-ref` with no value, an empty value, or immediately followed by another option — is rejected as a CLI usage error), and recursive detection of tracked placeholder files (`.gitkeep`, empty files, whitespace-only files, including governance files) across the entire Material root. It does not check empty directories — Git does not track them, so their presence or absence is not a repository fact — and does not parse imports, exports, CSS, or Vue/TypeScript source. Dependency-direction and public-import boundaries (Material must not import product layers; generic `shared/lib` must not depend on Material; external consumers must use the Material public API instead of a deep import) are **not** automated by `.oxlintrc.json`, `eslint.config.mjs`, or any other mechanism. They remain review-driven until real component migrations establish the actual public entry-point structure, whether family-level entry points are needed, the migration and compatibility-export model, and whether a general resolver-aware FSD boundary mechanism is justified — a hand-written alias/regex lint rule authored ahead of that evidence would encode a guess, not a stable contract.
...
1. Implement the minimal filesystem library-boundary baseline for new work — done (PR 151). Dependency-direction and public-import boundaries remain review-driven, not automated.
