...
- Minimum verification: run `pnpm verify --only static`, then run focused verify-managed VFS tests for the touched path, lock, and event behavior, plus browser-level checks when concurrency or event delivery changed.
