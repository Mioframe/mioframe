...
| M1  | Static Material architecture validation               | `done`    | M0                                          | minimal deterministic filesystem checks (exact canonical new-component placement, empty canonical directories, empty placeholder files and root barrel) plus ESLint/oxlint dependency-direction boundaries (alias and relative-path Material imports, `@shared/service` imports) block invalid new work, proven by tests that execute the real lint path; broader profile/style/test-artifact/story-identity/migration-residue checks are explicitly out of scope for this milestone, to be added later only when real migrations justify them |
...
- dependency-direction boundaries — Material must not import product layers by alias or by relative path, statically or through a dynamic `import()`; generic `shared/lib` must not depend on Material, statically or dynamically; external consumers (including product-layer files, which also keep their existing `@shared/service` boundary) must use the Material public API, statically or dynamically — are enforced through ESLint/oxlint `no-restricted-imports` for static imports/exports and real ESLint's `no-restricted-syntax` (targeting `ImportExpression`) for dynamic imports, since oxlint has no dynamic-import-aware rule, not a bespoke validator or parser; relative-path and root-alias matching is anchored so it blocks only actual product-layer roots and descendants, not unrelated paths that merely contain a layer name (e.g. `@shared/lib/features/helper`); proven by tests that execute the real lint path against fixtures rather than inspecting configuration JSON.
