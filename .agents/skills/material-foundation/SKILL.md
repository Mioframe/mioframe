...
description: 'Use for one real cross-family Material contract or an exact delegated prerequisite. Reconstructs current truth, canonicalizes the required owner, and returns to the calling family.'
---
...
Otherwise return `blocked`. Track nested prerequisites and stop only on an exact external blocker or cycle.
