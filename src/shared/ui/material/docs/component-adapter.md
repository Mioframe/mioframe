...
This document defines durable implementation rules for translating a canonical Mioframe Material family contract to the private renderer.

The public contract is owned by [`component-contract.md`](./component-contract.md). Workflow order is owned by [`component-workflow.md`](./component-workflow.md).
...
Never derive public API from m3e, legacy Mioframe components, or current consumer demand.
...
A renderer mapping is acceptable only when its observable result satisfies the canonical Material contract.
...
4. architecture/upstream escalation when faithful implementation would require private DOM coupling, duplicated renderer systems, new shared infrastructure, or weakening the public contract.
...
- explicitly forward only the public/native/ARIA allow-list required by the family contract;
...
When Material specifies fixed observable geometry, browser proof must measure the public rendered boundary numerically. A screenshot alone is not a geometry oracle.

Renderer-owned appearance and motion require faithful browser/visual evidence where the contract depends on them. Do not add host overrides that fight renderer-owned transient timing or geometry merely to match a screenshot.

## Accessibility and native behavior

Put semantics on the actual interaction owner.
...
- Preserve normal event propagation unless the canonical contract requires interception.
...
- a canonical contract requires the behavior;
...
1. consume the canonical contract without redesigning it;
2. inspect exact-version m3e documentation/public artifacts;
3. implement renderer mapping and standalone component;
4. add contract/browser/visual/token proof as required;
5. establish standalone correctness;
6. inventory and migrate consumers;
7. remove replaced legacy ownership and obsolete staged artifacts;
8. run focused verifier-managed checks.

When implementation evidence shows the family contract itself is incorrect, return the exact contract defect. Do not silently rewrite the contract while coding.
