...
- Switch: `MDSwitch` is `aligned`.
  ...
  Focus indicator: the global `useFocusIndicator` composable detects `data-md-focus-indicator-target` on the switch handle and renders the focus ring around the handle bounding box, matching the Material spec. No Switch-specific focus ring; the mechanism is generic and re-usable by other components.
