...
- Cards: `MDCard` is `partial`.
  ...
  Token status: `--md-comp-{elevated,filled,outlined}-card-*` implemented per variant for container (color, shape, elevation, shadow-color), disabled (container color/opacity for elevated/filled, outline color/opacity for outlined, container elevation for all three), focus indicator (color/thickness/offset), and hover/focus/pressed/dragged (state-layer color/opacity for all three; outline color for outlined only). Container elevation resolves to `--md-sys-elevation-levelN`; state-layer color/opacity forward into `MDStateLayer`'s generic `--md-private-state-layer-color`/`--md-state-*-layer-opacity` contract; focus indicator tokens forward into the generic `--md-focus-indicator-*` vars read by the global focus indicator. Icon-part tokens (`icon.size`/`icon.color`) and `container.surface-tint-layer.color` are not implemented — MDCard has no icon/media anatomy or tonal elevation surface-tint overlay in this pass.
  Public API status: `variant: 'elevated' | 'filled' | 'outlined'` (default `filled`), `mode: 'static' | 'button' | 'link'` (default `static`), `href`, `disabled`, `dragged`, `nativeType` props; `action` emit; default slot only. `static` renders a non-actionable `div` (no role, no tabindex, no state layer/ripple); `button`/`link` render a native `button`/`a[href]` that is the actionable surface itself, with `MDStateLayer` + `useRipple` and native keyboard activation.
  Interaction contract status: actionable cards do not support nested buttons/links as a documented pattern (Material's accessibility guidance avoids stacking actionable surfaces); this is documented in `shared/ui/Card/README.md` and Storybook notes rather than enforced with a pre-render scan of the (arbitrary) default slot. `mode="button"` renders the card as a native `<button>`, whose content model only accepts phrasing content — MDCard warns in development (`cardDevWarnings.ts`) when rendered content includes a block-level or interactive tag, which also incidentally catches nested buttons/links. `mode="link"` has no such restriction (`<a>` accepts flow content). Disabled `button` cards use native `disabled`; disabled `link` cards use `aria-disabled="true"` + `tabindex="-1"` plus a script-level click guard, since links ignore native `disabled`.
  ...
  Storybook status: `shared/ui/MDCard` documents variants, a static card with internal actions, actionable button/link cards, an action/keyboard-behavior story with observable click counters, disabled actionable cards, a dragged card, a root-level forced interaction-state gallery (`md-state_*` class fallthrough per variant, including dragged) per variant, and a visual state gallery (static/actionable/disabled/dragged per variant).
  Visual or browser verification status: `MDCard.test.ts` covers default variant/mode, per-variant modifier classes, static non-actionable behavior (no role/tabindex/state layer, no `action` emit), button mode (native `button`, `type`, `action` emit, disabled), link mode (native anchor, `href`, `action` emit, disabled aria-disabled/tabindex), state-layer presence gated by mode, and the button-mode phrasing-content dev warning (fires for block content, silent for phrasing content, silent for static/link modes). `tests/e2e/visual/shared-ui.spec.ts` covers the visual state gallery, the root-level forced interaction-state gallery, static-card non-actionable DOM contract (including no direct-child state layer/ripple), actionable button-card click/Enter/Space activation with an observable counter, actionable link-card click/Enter activation with an observable counter and fragment navigation, disabled link-card blocked-navigation/no-action semantics, and root-level Material surface-context token propagation. Verified through the canonical Podman container flow (`pnpm test:visual`); the interaction-state baseline (`md-card-interaction-states-linux.png`) was regenerated through `pnpm test:visual:update` and inspected to confirm it now shows real root-level elevation and outline changes, not only the nested state layer.
  Deviations or unsupported features: no icon/media anatomy, no `container.surface-tint-layer.color` tonal elevation overlay, and no support for nested actionable content inside an actionable card (documented, and now also caught in development by the button-mode phrasing-content warning for `mode="button"`). Internal padding/gap (16dp padding, 8dp content gap) is a project layout default, not an official Material token.
  Card text anatomy status: MDCard has no headline/supporting-text slots or `title`/`description` props. Consumers keep their own semantic tags and apply the global `.md-typescale-*` type scale utility classes from `shared/lib/md/typography.css`; card status stays `partial` for the full Cards family (media, actions row, tonal overlay remain unimplemented) even though the surface and this typography contract are aligned.
