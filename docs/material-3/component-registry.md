...
- Lists: `MDList`, `MDListItem`, and `MDListContainer` are `partial`.
  ...
  Shape status: `MDListContainer` defaults `--md-comp-list-container-shape` to `0dp`, matching the Material baseline standard list style (square corners). The expressive/segmented style (rounded corners) is not implemented. Consumers that need a rounded container (e.g. `DatabaseViewWidget`) explicitly set their own `--md-comp-list-container-shape` value. The dead first/last-child corner-radius rules in `MDListItem` have been removed since they always evaluated to `0dp` under the baseline default.
  Public API status: explicit `static`, `single-action`, `multi-action`, `single-select`, and `multi-select` modes with Material anatomy slot names. `single-action` requires either an `@action` listener or an `href`; a dev-mode warning fires when neither is present. `multi-action` requires a real primary action and a `#trailingAction` slot; a dev-mode warning fires when the slot is absent. Static rows with trailing controls use `static` mode with the `trailing` slot. `MDListContainer` and `MDList` expose `layout?: 'column' | 'grid'` as a project-specific layout helper that is not a Material list style. Material list styles (`standard`, `segmented`) are not yet exposed as a distinct API variant.
  ...
  DOM structure status: `MDListContainer` renders `div[role=list]` (or `ul`). All `MDListItem` modes render with an explicit `role="listitem"` on the root element when the root tag is not `li` (which carries the implicit listitem role). This covers all modes: static (`div[role=listitem]`), single-action (`button[role=listitem]` or `a[role=listitem]`), multi-action (`div[role=listitem]`), and select (`div[role=listitem]`). Secondary controls are not nested inside the primary-action element.
  ...
  Storybook status: `Material 3/Components/Lists/MDListItem` documents configurations (including overline), visual states, interaction states, multi-action layout, and selection. All `multi-action` stories include a primary `@action` handler and a `#trailingAction` slot. The interaction states story shows single-action and multi-action state coverage side by side; supporting text describes surface coverage, not opacity values. State-layer color is set to `var(--md-sys-color-primary)` in the story wrapper for visual clarity.
  Visual or browser verification status: `Configurations`, `VisualStates`, `VisualInteractionStates`, and `TrailingActionLayout` stories are tagged `visual`. Interaction states story covers both single-action (full-row state layer) and multi-action (primary-action-bounded state layer). Browser-level Playwright assertion verifies trailing action icon buttons expose a `.md-icon-button__target` span of at least 48×48 px.
  Deviations or unsupported features: expressive/segmented list style (rounded corners, gaps between items, highlighted selection), list-level selection semantics (listbox/option ARIA, roving keyboard), and shared roving-keyboard handling for multi-action rows are not implemented. Lists remain `partial`.
