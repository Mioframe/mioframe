...
| 2026-07-16 | M1        | Removed all Material-specific import-boundary lint automation added earlier in PR 151 (`.oxlintrc.json` `no-restricted-imports` overrides, the `local/no-restricted-dynamic-imports` ESLint rule and its plugin registration, and `scripts/materialLintBoundary.test.mjs`): those boundaries were encoded before any real migration established a stable public entry-point/compatibility model, so they are reverted to review-driven rather than kept as a second automation mechanism; `scripts/materialStaticValidation.mjs` and the `material-static` verify command, the genuine minimal filesystem baseline, are unaffected and M1 remains `done` on that narrower exit gate | `feat/material-static-architecture-validation`      |
