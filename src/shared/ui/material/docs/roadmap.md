...
- the demand-scoped API covers the current uncontained short-loading scenario and required accessible purpose labeling: `label` (required) and a constrained numeric `size` (dp mapped to px, default `48`, clamped to the official `24..240` range with a development-mode warning on out-of-range input);
...
- colocated contract tests, an independent standalone visual matrix (`24`/`32`/`40`/default `48`, plus inherited color), and standalone real-browser accessibility proof (`tests/e2e/storybook/md-loading-indicator.spec.ts`: browser-resolved `progressbar` role and accessible name) exist; operator visual/motion review is pending.
