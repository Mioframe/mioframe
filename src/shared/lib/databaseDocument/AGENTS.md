...
- Minimum verification: run `pnpm verify --only static`, then verify the touched schema path against validation, migration from an older payload, and effective-value behavior when defaults are involved through focused verify-managed tests.
