...
This runs a minimal, deterministic filesystem validator (`scripts/materialStaticValidation.mjs`) over `src/shared/ui/material`. It is a filesystem-only baseline, not a complete architecture framework: new official Material components must be created under the exact canonical `components/<family>/MD*.vue` path (diff-aware, grandfathers pre-existing legacy files, and stays disabled only when no comparison base ref is supplied at all — an explicitly supplied `--base-ref` that git cannot read fails the run instead of silently skipping this check, and a malformed invocation — `--base-ref` with no value, an empty value, or immediately followed by another option — is rejected as a concise CLI usage error before any check runs), and empty canonical directories (including the `components`/`foundation`/`patterns` namespace roots themselves), empty or whitespace-only placeholder files (including governance files such as `README.md`), and an empty root barrel are rejected. It does not parse imports, exports, CSS, or Vue/TypeScript source.

Dependency-direction rules (Material must not import product layers by alias, statically or dynamically; generic `shared/lib` must not depend on Material by alias, statically or dynamically; external consumers, including product-layer files, must use the Material public API by alias instead of a deep import, statically or dynamically) are enforced through ESLint/oxlint `no-restricted-imports` overrides in `.oxlintrc.json` for static imports/exports, and through a small dedicated ESLint rule (`local/no-restricted-dynamic-imports`, visiting only `ImportExpression` string literals) in `eslint.config.mjs` for dynamic `import()` calls, since oxlint has no dynamic-import-aware rule and reusing `no-restricted-syntax` for this would silently replace the unrelated Vue DOM-communication restrictions (`dispatchEvent`/`querySelector`) configured for overlapping files. Alias and root-alias matching is anchored so it blocks only actual product-layer roots and descendants, not unrelated paths that merely contain a layer name. Relative-path layer imports are not resolved and are not automatically enforced, statically or dynamically; they remain review-driven, intentionally, to avoid a speculative path resolver or a false-positive-prone regex policy. These are part of the existing `oxlint`/`eslint` verify steps, not this validator, and are proven by tests that execute the real merged lint configuration against fixtures.
