...
2. resolve an added, changed, or deleted Vitest snapshot to its owning test through the configured snapshot convention;
3. for changed existing non-test modules, including production files and local test helpers/fixtures imported by tests, use Vitest related-test selection based on static imports;
4. use a full `unit-tests` fallback for an unresolved snapshot, deleted or renamed non-test module, Vitest config/setup, global test utility, known dynamic-import boundary, generated alias, or any relation Vitest cannot represent safely;
5. union and deduplicate direct, snapshot-owned, and related test paths;
6. do not maintain a second generic dependency graph.
...
### Playwright: lane-specific impact registries
...
Each lane also declares:

- its spec directory;
- broad stable `relevantSourcePrefixes` where an unmapped source change can affect the lane;
- exact files and prefixes that always require the full lane;
- its mappings and justified standalone specs.

These lists are mechanical execution ownership, not a second product architecture. Add a relevant prefix only for a stable source domain, not for one task or one file.
...
Entry names, arrays, specs, and reasons are non-empty. For each changed path, resolve in this order:

1. a full-lane exact file or prefix selects the complete lane;
2. a changed spec selects itself and any additional specs selected by mappings that match its path;
3. an added, changed, or deleted visual baseline selects its owning visual spec and any mappings that match that spec; an unresolved baseline selects the full visual lane;
4. a changed mapped source selects the union of all matching specs;
5. an unmapped source under the lane’s `relevantSourcePrefixes` selects the complete lane;
6. a path outside the lane’s spec, snapshot, mapping, relevance, and full-lane domains does not select that lane.

Across multiple changed paths, union and deduplicate all selected specs; a full-lane match wins.

Validation and safety rules:

1. every discovered spec is covered by a mapping or a justified standalone entry;
2. a new, moved, or removed Playwright spec updates its owning mapping or standalone list in the same change;
3. a shared lane config, global fixture, or common helper selects the complete owning lane unless all consumers are explicit and validated;
4. missing specs, duplicate mapping names, duplicate paths within one entry, duplicate standalone specs, empty reasons, invalid paths, or broken registry coverage fail closed;
5. overlapping source prefixes and the same spec referenced by multiple mappings are valid;
6. do not create a cross-lane dependency graph, production annotations, or a generic test DSL.
...
- changed test/spec or resolvable snapshot: owning test/spec plus matching mappings;
- changed common helper/config: full owning lane;
- unresolved snapshot, deleted/renamed unit dependency, or unknown unit relation: full unit lane;
- unmapped source inside a Playwright lane’s relevance domain: full lane;
- path outside a lane’s declared relevance: no selection for that lane;
...
12. a relevance prefix exists only to support one task or masks stable mappings;
