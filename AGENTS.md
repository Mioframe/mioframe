...
- Readiness or hydration checks must be exposed by the owner of the persisted/source state as a domain-specific readiness field. Upper layers must not reach into a lower-level storage lifecycle composable only to gate a feature; depend on the owning entity/feature API instead.
