...
- `tests/e2e/appSmoke.spec.ts` -> real invocation is `vitest related playwright.lanes.test.ts scripts/lib/e2eProjectApplicability.test.ts scripts/lib/e2eRisk.test.ts` (the spec path itself never passed to Vitest), confirming Playwright inventory scan ownership (mechanism 5) without crossing the Playwright/Vitest ownership boundary. Re-verified after the M1 correction (narrow `isPlaywrightLaneInventoryScanPath` predicate replacing the over-broad `isPlaywrightOnlyProofPath` reuse): all three owners are selected and all 110 tests across the three files pass;
- `src/shared/service/fileSystem/directoryState.ts` (post-sync-introduced production source, representing the synchronized directory/repository-state develop change) -> real invocation is `vitest related src/readRecoveryImportBoundary.test.ts src/shared/service/fileSystem/directoryState.ts src/shared/ui/material/rendererBoundary.test.ts` (both bounded-scan owners additive to the source's own ordinary pass-through), and all 54 selected test files (680 assertions) pass, confirming the final table corresponds to the post-sync tree rather than the pre-sync baseline.

### Pass C post-sync correction refresh (B1 status-aware exact ownership, B2 Playwright lane contract, M1 narrow scan predicate)

Scope: only the unit lane is affected by this correction; Pass B/D/E resolver logic was not touched, so their columns are not re-derived here. Cases below use the same method as the main benchmark table (direct `resolveUnitPlan` invocation, real local `performance.now()` timing), plus a real focused verifier invocation for the two cases requiring delegated-resolver proof (Playwright inventory and the post-sync production source, both already covered under "Real unit resolver proof" above).

| Case                                                                          | unit (planner)                                                                                                                                                                                                                                                                                          | planner time |
| ----------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ |
| deleted exact external input `.github/workflows/verify.yml`                   | **focused** -> all 5 surviving exact owners (`ciAutofix.test.ts`, `buildDateWorkflow.test.mjs`, `managedDeploymentValidationWorkflow.test.mjs`, `materializePrVersionWorkflow.test.mjs`, `scripts/verify.test.ts`); no longer `skip`                                                                    | ~1.0ms       |
| renamed exact external input `PRIVACY.md` (old side) -> `PRIVACY.archived.md` | **focused** -> `DataStoragePrivacyPane.test.ts` via the old path; no longer `skip`                                                                                                                                                                                                                      | ~0.2ms       |
| Playwright root app inventory `tests/e2e/appSmoke.spec.ts`                    | **focused** -> `playwright.lanes.test.ts`, `e2eRisk.test.ts`, `e2eProjectApplicability.test.ts` (never the spec itself); real invocation green, 110/110 tests passing across the three owners (see "Real unit resolver proof")                                                                          | ~0.2ms       |
| adjacent nested negative `tests/e2e/other/example.spec.ts`                    | **skip** -- `.spec.ts`-shaped and under `tests/e2e/`, so it matches the old over-broad `isPlaywrightOnlyProofPath`, but the corrected narrow `isPlaywrightLaneInventoryScanPath` predicate correctly excludes it (outside all six real scanned populations); `playwright.lanes.test.ts` is not selected | ~0.1ms       |
| post-sync production source `src/shared/service/fileSystem/directoryState.ts` | **focused** -> self + `readRecoveryImportBoundary.test.ts` + `rendererBoundary.test.ts` (both bounded scans); real invocation green, 54 test files / 680 tests passing                                                                                                                                  | ~0.1ms       |

Deleted/renamed `.gitignore` was also proven at the planner level (`deleted('.gitignore')` and `renamed('.gitignore', '.gitignore.disabled')` both resolve `focused -> scripts/agentEnvironment.test.mjs`, never `skip`) as part of the fresh test-author proof in `scripts/lib/unitRisk.test.ts`'s "resolveUnitPlan status-aware exact external ownership" describe block; not independently re-benchmarked here since it is the same mechanism as the `.github/workflows/verify.yml` and `PRIVACY.md` cases above.

**B2 (Playwright lane contract):** `playwright.lanes.test.ts` previously asserted a top-level `appConfig.testIgnore` that no longer exists in production `playwright.config.ts` (the config sets `testIgnore` per-project only). The test now resolves the real `chromium`/`Mobile Chrome` projects by name (via `DESKTOP_PROJECT_NAME`/`MOBILE_PROJECT_NAME` from `scripts/lib/e2eProjectApplicability.ts`) and asserts each project's `testIgnore` contains `storybook/**`, `visual/**`, `release/**`, without duplicating `e2eProjectApplicability.test.ts`'s own registry contract. `playwright.config.ts` was not changed. Confirmed green via `pnpm verify --only unit-tests --files playwright.lanes.test.ts`.

**Post-sync semantic ownership audit result:** `new relations: none`. The dedicated test-author context re-audited the complete current Vitest-owned test population (node:fs reads/existence checks, readdir/bounded scans, runtime/tool config discovery, equivalent external observations) after the develop sync, explicitly including the sync-introduced `src/shared/service/fileSystem/directoryState.ts`/`.test.ts` and `src/shared/service/repositories/repositoryState.ts`/`.test.ts`/`.integration.test.ts` (none use `node:fs` or any external-observation mechanism), plus every other grep hit not already covered by the existing top-of-file audit in `scripts/lib/unitRisk.test.ts` (each individually re-read and classified as temporary/test-owned fixture data, redundant-with-real-import ordinary ownership, or mocked/no-op). No new exact-external, bounded-scan, or unit-global-infrastructure relation was found.
...
The post-sync correction round closed three further false negatives/over-selections identified in `scripts/lib/REVIEW.md` (B1, B2, M1):

- exact external ownership (`PRIVACY.md`, `.github/workflows/*.yml`, `.gitignore`) was previously consulted only in the `added | modified` branch of `resolveUnitPlan()`; a deleted or renamed exact-mapping-only source (none of these are ordinary-unit-source-shaped) silently resolved to `skip`, dropping its surviving owner. `resolveUnitPlan()` now evaluates the exact mapping lookup for every changed-path side (added/modified/deleted, and both sides of a rename), additive to the existing scan-owner and infrastructure-trigger checks, and independent of current source existence on disk;
- `playwright.lanes.test.ts`'s scan-owner predicate in `UNIT_SCAN_OWNERS` reused `isPlaywrightOnlyProofPath`, which matches any `.spec.ts` at arbitrary nesting depth under `tests/e2e/`, over-selecting the lane test for a hypothetical `tests/e2e/<other-subtree>/*.spec.ts` outside its real six-population scan. A dedicated `isPlaywrightLaneInventoryScanPath` predicate now mirrors exactly the lane test's real filesystem scan; `isPlaywrightOnlyProofPath` itself is unchanged and still owns the separate ordinary-Vitest-exclusion responsibility;
- `playwright.lanes.test.ts` itself was stale against the current `playwright.config.ts` (asserting a removed top-level `testIgnore`), which would have made the Playwright-inventory real-resolver acceptance case red; corrected per B2 above.
