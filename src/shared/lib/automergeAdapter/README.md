...
- V3 `.mf` filenames participate in routing.
...
- The primary generated v3 filename is `<docPrefix>.<kindCode>.<fingerprint>.mf`, where
  ...
  contains a hash prefix; the filename participates in routing, and the wrapper's full
  `StorageKey` confirms the logical key after filename routing. Normal writes always resolve to
  this one deterministic filename and never generate a numeric-suffix or copy-style fallback name.
- Exact `load` and `save` for a full chunk key resolve the primary filename directly,
  ...
  - `remove` performs one fresh `listNames()` classification pass for a full chunk key, prefilters
    strict v3 `.mf` candidates by the requested filename route, wrapper-confirms only those
    in-route candidates with bounded concurrency, and removes every supported physical file whose
    confirmed full logical key matches the requested key. This includes the primary v3 file,
    released v2, released legacy with `.automerge`, and released extension-less legacy variants. It
    never reads or removes out-of-route `.mf` files, never removes an invalid `.mf` wrapper
    blindly, and never removes a valid wrapper for a different key. Marker-file removal remains
    marker-only.
