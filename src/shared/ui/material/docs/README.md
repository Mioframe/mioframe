...
The owning README contains one coherent workflow-state block plus the applicable canonical target, source decisions, current assessment, alignment map, dependency classifications, decomposition, public token surface, private routing responsibilities, style and motion contracts, proof map, correction units, and remaining gaps.

Exact selectors, CSS declarations, token route lists, keyframe references, and runtime animation routes remain owned by code. Auditors and reviewers reconstruct them on every relevant run instead of maintaining a duplicate code ledger in documentation.
