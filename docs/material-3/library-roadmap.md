...
| M1  | Static Material architecture validation               | `done`    | M0                                          | minimal deterministic filesystem checks (exact canonical new-component placement, recursive tracked-placeholder-file detection across the Material root) block invalid new work, proven by tests, and are wired into the standard `pnpm verify` gate; dependency-direction and public-import boundaries are not automated and remain review-driven; broader profile/style/test-artifact/story-identity/migration-residue checks are explicitly out of scope for this milestone, to be added later only when real migrations justify them |
...
This milestone's exit gate is the complete minimal baseline:
...
- every tracked file under `src/shared/ui/material`, at every depth including the root, is checked recursively for placeholder content: `.gitkeep` files and empty or whitespace-only files (including governance files such as `README.md`/`AGENTS.md`/`CLAUDE.md`) are rejected; Git does not track empty directories, so directory emptiness is not a repository fact and is not validated;
- the validator is wired into `pnpm verify` as the light `material-static` command, receiving `--base-ref` only when a reliable comparison ref exists.

Out of scope for this milestone, and not required for it to be `done`:

- dependency-direction and public-import boundaries — Material must not import product layers, generic `shared/lib` must not depend on Material, external consumers must use the Material public API instead of a deep import — are **not** automated by lint configuration or any other mechanism. They remain review-driven: checked by the coding agent and human review, the same as any other repository convention without a static check;
...
Dependency-direction and public-import automation is deliberately deferred, not merely unautomated: introducing it now would guess at a public entry-point structure, a family-level entry-point need, and a migration/compatibility-export model before any real migration has established them. Only real component migrations (starting with `MDButton` in M6) can supply that evidence. When they do, that evidence should also settle whether a general resolver-aware FSD boundary mechanism is justified, rather than another hand-written alias/regex lint rule. See `docs/material-3/token-validation.md` for the full deferred-check catalogue.

Implementation status: minimal baseline implemented as `scripts/materialStaticValidation.mjs`, wired into `pnpm verify` as the `material-static` label. This milestone's exit gate is complete; M2–M10 do not depend on any further M1 automation, and M1 is not reopened by deferring import-boundary automation to a future milestone.
...
| 2026-07-16 | M1        | Established canonical placement validation for newly added official components, recursive tracked-placeholder-file detection across the Material root, comparison-base handling, and standard `pnpm verify` integration; dependency and public-import boundaries remain review-driven; marked `done` | `feat/material-static-architecture-validation`      |
