...
- Minimum verification: run `pnpm verify --only static`, then verify each touched path with old-version input, already-latest input, and once-only upgrade behavior through focused verify-managed tests.
