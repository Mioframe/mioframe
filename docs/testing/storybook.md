...
| Visual spec and baseline        | mixed discovery is executable: Loading Indicator (Stage S3, complete), Chips (Stage S4-A, complete), MDCheckbox (Stage S4-B, complete), and MarkdownContent (Stage S4-C, complete) use filesystem-derived owner-local `src/**/*.visual.spec.ts` ownership with colocated snapshots; all other visual specs/baselines remain under `tests/e2e/visual` with full-lane fallback until their own Stage S4 group |
...
Loading Indicator (Stage S3, complete), Chips (Stage S4-A, complete), MDCheckbox (Stage S4-B, complete), and MarkdownContent (Stage S4-C, complete) are executable through this convention. The visual resolver must handle added, modified, deleted, and renamed specs/baselines. If ownership cannot be determined exactly, select the full visual lane.
