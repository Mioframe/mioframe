...
- Browser-only user actions such as a picker or permission prompt may live in a feature, but the feature must report the result back to the owning entity/service instead of becoming the source of truth.
...
- Do not store canonical provider, repository, document, or filesystem state inside a feature.
- Do not pass service bags, provider instances, or capability handles through feature props unless the feature is explicitly the narrow user-action boundary for that capability.
