...
function getForkPoint(baseRef) {
  return (
    runGitCommand(['merge-base', '--fork-point', baseRef, 'HEAD'], { allowFailure: true })[0] ??
...
function getChangedFiles() {
  ...
  if (githubBaseRef) {
    ...
    if (
      ...
      spawnSync('git', ['rev-parse', '--verify', '--quiet', 'HEAD~1']).status === 0
    ) {
      changedFiles = runGitCommand([
        'diff',
        '--name-only',
  ...
  return {
    changedFiles: uniqSorted(
      changedFiles.map(toPosixPath).filter((filePath) => !hasPrefix(filePath, IGNORED_PREFIXES)),
    ),
    scope,
  };
}
      ...
      fs
        ...
        .filter(
          (entry) =>
            ...
            entry.name.slice(0, -'.test.ts'.length).split('.')[0] === baseName,
        )
...
function getMutationSourceCandidate(testFilePath) {
  ...
  for (const candidateBase of [
    basePath,
    `${dirPath}/${baseName.split('.').slice(0, -1).join('.')}`,
  ]) {
...
function getMutationScope(changedFiles) {
  ...
  for (const filePath of changedFiles) {
    if (filePath.startsWith('src/') && filePath.endsWith('.test.ts')) {
      ...
      filePath.startsWith('src/') &&
      !filePath.startsWith('src/shared/ui/') &&
      SOURCE_EXTENSIONS.includes(path.posix.extname(filePath)) &&
      getAllSiblingTestFiles(filePath).length > 0
    ) {
      scope.push(filePath);
    }
  }
...
function appendToRollingBuffer(buffer, chunk) {
  ...
  return nextBuffer.length <= MAX_ROLLING_BUFFER_CHARS
    ? nextBuffer
    : nextBuffer.slice(-MAX_ROLLING_BUFFER_CHARS);
}
...
function summarizeCommandForDisplay(command, args) {
  ...
  return formatCommand(command, [
    ...otherArgs,
    ...groupedFileArgs.slice(0, MAX_FILE_ARGS_IN_SUMMARY),
    ...(remainingCount > 0 ? [`<+${remainingCount} files>`] : []),
  ]);
}
...
function buildCommands(changedFiles) {
  ...
  const formatLintFiles = existingChangedFiles.filter(
    (filePath) => !hasPrefix(filePath, FORMAT_LINT_IGNORED_PREFIXES),
  );
  ...
  commands.push(
    changedFiles.some(isTypeCheckTarget)
  ...
  commands.push(
    changedFiles.some(isVisualRelevantFile) || changedVisualSpecs.length > 0
...
function selectCommands(commands) {
  ...
  return onlyLabels.map(
    (label) =>
      commandByLabel.get(label) ?? {
        kind: 'skipped',
        label,
        command: DISPLAY_COMMAND_BY_LABEL.get(label) ?? `pnpm verify --only ${label}`,
        reason:
          label === 'e2e-install' && e2eCommand?.kind === 'skipped'
            ? e2eCommand.reason
            : 'check is not part of the current verify mode',
      },
  );
}
