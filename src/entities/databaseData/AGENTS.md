...
- Minimum verification: run `pnpm verify --only static`, then edit data in an active view and confirm refresh or reopen keeps the same row set for the touched filter, sort, and default-value semantics. Use focused verify-managed tests where available.
