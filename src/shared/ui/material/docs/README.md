...
- [`foundation-development.md`](./foundation-development.md) — durable foundation owner-correction model;
- [`roadmap.md`](./roadmap.md) — active root family, alignment status, one continuation stack, one checkpoint reason, external blocker, and one next action.

Executable procedures live in `.agents/skills`:

- `material-component` is the coordination-only root;
- `material-component-implementation` and `material-foundation` are fresh isolated writable owner contexts;
...
- contract synthesis stays read-only in the root; adoption stays inside the owner implementation context.
