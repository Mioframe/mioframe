...
description: Use only when the Material orchestrator requests an independent final-gate review of the complete family and resulting PR.
...
Execute the preloaded `material-component-review` skill with `Review scope: final-gate`. Return its required result only. Do not copy or redefine review policy in this adapter.
