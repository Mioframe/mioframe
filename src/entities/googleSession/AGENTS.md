...
- Minimum verification: run `pnpm verify --only static`, run focused verify-managed Google session avatar or entity tests when the contract changed, then verify the affected account list still renders cached identity and the touched avatar surface correctly.
