...
- Switch: `MDSwitch` is `partial`, first implementation.
  Material docs checked: official `components/switch/*` pages are not present in the `material3` MCP cache (confirmed via direct page lookup and a cache refresh on 2026-06-26); only the cached selection-state foundation pages (`foundations/interaction/selection.md`, `foundations/interaction/states/applying-states.md`) confirm Switch participates in the standard Material interaction-state model.
  Token status: reuses `--md-sys-color-primary`, `--md-sys-color-on-primary`, `--md-sys-color-outline`, `--md-sys-color-on-surface`, `--md-sys-color-surface` directly, matching the existing `MDCheckbox` selection-control token pattern; no dedicated `--md-comp-switch-*` tokens added pending verified spec values.
  ...
  Interaction contract status: click/Enter/Space toggle in interactive mode; presentation mode is non-interactive and used by row-level consumers (e.g. `SettingsSwitchListItem`) that own the action themselves, matching the existing `MDCheckbox`-in-`SettingsCheckboxListItem` pattern.
  Storybook status: `shared/ui/MDSwitch` documents default/on/disabled states and visual/interaction-state galleries.
  Visual or browser verification status: `MDSwitch.test.ts` covers keyboard/pointer/disabled/focus/autofocus contracts; visual baselines added to `tests/e2e/visual/shared-ui.spec.ts` pending generation through the canonical container flow.
  Deviations or unsupported features: track/thumb sizing (52x32dp track, 16dp/24dp thumb) and state-layer shape (track-shaped overlay rather than a 40dp thumb-following circle) are assumed from common Material switch conventions, not verified against an official spec page, since none is cached. No selected-state icon is rendered. Treat exact dimensions and the state-layer shape as an unresolved Material 3 verification risk until official switch docs become available in the MCP cache.
