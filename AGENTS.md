...
- Repo-local Codex hooks in `.codex/` automatically preload prompt-matched project memory and run stop-time lifecycle review. They may request one extra pass, `SessionStart` and `UserPromptSubmit` may fail-soft, and `PreToolUse` / `Stop` fail closed on internal errors; they still do not replace `pnpm memory:task:start` or `pnpm memory:task:finish`, and canonical discovery still starts at `memory:task:start`.
