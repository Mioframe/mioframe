...
- Minimum verification: run `pnpm verify --only static`, then mount or disconnect the touched directory source through the existing feature and confirm the visible state updates in the consuming screen. Use focused verify-managed coverage where available.
