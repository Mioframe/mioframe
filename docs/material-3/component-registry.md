...
- Lists: `MDList`, `MDListItem`, and `MDListSelectionItem` are `partial`.
  Material docs checked: `components/lists/overview.md`, `components/lists/specs.md`, `components/lists/guidelines.md`, `components/lists/accessibility.md`, `foundations/design-tokens/overview.md`, `foundations/interaction/states/state-layers.md`, `styles/motion/overview/how-it-works.md`, and `styles/motion/overview/specs.md` (captured 2026-06-30).
  ...
  Motion status: `animateMoves` uses the M3 Expressive motion physics system's **fast spatial** Web conversion (`cubic-bezier(0.42, 1.67, 0.21, 0.90)`, `350ms`) for the FLIP `transform` move and the dragged row's border-radius, and **fast effects** (`cubic-bezier(0.31, 0.94, 0.34, 1.00)`, `150ms`) for the dragged row's box-shadow elevation — both verified against `styles/motion/overview/specs.md`. No dedicated list-reorder duration token is documented; these are the general "small component" fast-speed spring conversions applied to reorder as spatial movement. `prefers-reduced-motion: reduce` disables the move transform transition only. Segmented `MDList` no longer clips (`overflow: clip` removed) and a dragged `MDListItem` gets `z-index: 1` so its elevation renders above sibling rows instead of being cut off.
