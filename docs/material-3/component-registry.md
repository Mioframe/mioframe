...
- Switch: `MDSwitch` is `partial`, first implementation.
  Material docs checked: official `components/switch/*` pages are still not present in the `material3` MCP cache (confirmed via direct page lookup and a cache refresh on 2026-06-26, re-confirmed via `search_material_docs` on 2026-06-27); only the cached selection-state foundation pages (`foundations/interaction/selection.md`, `foundations/interaction/states/applying-states.md`) confirm Switch participates in the standard Material interaction-state model.
  Token status: track width/height/shape, track border width, and thumb sizes/offsets are private implementation variables (`--md-switch-track-width`, `--md-switch-track-height`, `--md-switch-track-shape`, `--md-switch-track-border-width`, `--md-switch-thumb-size-unselected`, `--md-switch-thumb-size-selected`, `--md-switch-thumb-offset-unselected`, `--md-switch-thumb-offset-selected`), authored in `dp` per the project unit pipeline, not public `--md-comp-switch-*` tokens. Color resolves directly from `--md-sys-color-primary`, `--md-sys-color-on-primary`, `--md-sys-color-outline`, `--md-sys-color-on-surface`, `--md-sys-color-surface`, matching the existing `MDCheckbox` selection-control token pattern. No `--md-comp-switch-*` tokens are defined because no official token path is cached; this is a documented missing-token gap, not a finished component-token contract.
  Public API status: `modelValue`, `disabled`, `id`, `ariaLabel`, `autofocus`, `tabIndex`, `presentation` props; `update:modelValue` and `click` emits. Mirrors `MDCheckbox`'s interactive/presentation dual-root contract.
  Interaction contract status: the focusable host (a `<label>` with `role="switch"`, `aria-checked`, and `aria-disabled`) owns click/Enter/Space activation and intentionally prevents native label-to-input click forwarding, driving the bound `v-model` checkbox purely as a hidden state mirror. This deliberately mirrors `MDCheckbox`'s existing interactive contract (including Enter-as-toggle, which is not native checkbox behavior but is the established and accepted project pattern for this dual-root selection-control family) rather than moving activation onto the native `<input>`. Presentation mode renders a non-interactive `aria-hidden` div with an explicit `md-switch_presentation` modifier (`pointer-events: none`, `cursor: default`) so it never shows a pointer affordance, and is used by row-level consumers (e.g. `SettingsSwitchListItem`) that own the action themselves, matching the existing `MDCheckbox`-in-`SettingsCheckboxListItem` pattern.
  Storybook status: `shared/ui/MDSwitch` documents default/on/disabled states and a visual/interaction-state gallery. The interaction-state gallery uses the story/test-only `MDStateLayerForcedStateProvider` (from `shared/ui/State/testing`) to force hover/focus/pressed visuals instead of applying internal `md-state_*` classes as an external API.
  Visual or browser verification status: `MDSwitch.test.ts` covers ARIA/role/disabled/presentation/keyboard/pointer/focus/autofocus component contracts (including the `md-switch_presentation` modifier class); `SettingsSwitchListItem.test.ts` was rewritten to use `@vue/test-utils` `mount` instead of hand-rolled `createApp`/manual DOM/`querySelector` assertions. Visual baselines exist in `tests/e2e/visual/shared-ui.spec.ts` (`MDSwitch visual states match baseline`, `MDSwitch interaction states match baseline`) pending generation/confirmation through the canonical container flow.
  Deviations or unsupported features: track/thumb sizing (52x32dp track, 16dp/24dp thumb) and state-layer shape (track-shaped overlay rather than a 40dp thumb-following circle) are assumed from common Material switch conventions, not verified against an official spec page, since none is cached. No selected-state icon is rendered. Treat exact dimensions and the state-layer shape as an unresolved Material 3 verification risk until official switch docs become available in the MCP cache. Switch remains `partial`, not `aligned`.
