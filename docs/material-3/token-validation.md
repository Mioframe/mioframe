...
A generated allowlist may cache verified token names for performance only when it is reproducible from a named source snapshot.
...
- canonical component tokens outside the owner file named by the family blueprint;
...
- family token selectors that differ from the exact applicable root list;
...
- tokens conditionally declared only for an active variant, size, shape, width, density, mode, or state.
...
Read the selected profile from the family blueprint.

### `simple`

Require `.vue`, `.tokens.css`, and `.css`. Reject `.routes.css` and `.states.css` unless the profile changes.

### `configured`

Require `.vue`, `.tokens.css`, `.routes.css`, and `.css`. Reject `.states.css` unless a stateful property route exists and the profile changes.

### `stateful`

Require `.vue`, `.tokens.css`, `.routes.css`, `.states.css`, and `.css`.

For every profile, verify exact style order and reject additional production files not permitted by the blueprint and architecture rules.
...
- `Readiness: ready` with `TBD`, unresolved alternatives, or missing required decisions;
- stateful rendered variable without a matrix row;
- matrix row without an implemented final value or actual property owner;
- implemented winner or coexistence rule absent from the matrix;
- optional family token, anatomy, behavior, composable, or context file that does not satisfy its objective extraction condition;
- production, Storybook, registry, or verification that disagrees with the accepted blueprint.
...
- supported capability missing required reachable states or accessibility behavior;
- project extension without explicit blueprint ownership;
- unsupported behavior presented as aligned.

These checks may require review evidence rather than pure syntax, but completion must still record them.

## Validation process

### Before implementation

1. run advisory inventory against the relevant legacy or new family area;
...
5. verify that the selected profile is the smallest applicable profile.
...
5. reject obsolete parallel routes and unapproved files.
...
- duplicate ownership and private-variable escape;
- system-token bypass;
...
- explicit unsupported features, deviations, and remaining verification gaps.
