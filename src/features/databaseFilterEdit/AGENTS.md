...
- Minimum verification: run `pnpm verify --only type-check`, then add or edit the touched filter shape, save it, reopen the same view, and confirm both the rendered chips and the resulting row set still match. Use focused verify-managed coverage where available.
