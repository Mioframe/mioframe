...
- Pages used: `overview.md`, `guidelines.md`, `accessibility.md` — inspected directly through the `material3` MCP local documentation cache (`m3.material.io` source, cache captured `2026-06-30T05:53:04.916Z`, per-page `capturedAt: 2026-06-30T05:48:50.423Z`). `specs.md` was cross-verified through the same MCP cache's structured component token-table graph (numeric sizes, paddings, icon sizes, shapes, and elevation/motion token names) rather than the raw markdown page, which exceeded single-fetch handling in this session; every numeric value used below was checked against that token-table graph.
...
- Official capability inventory: `complete` — every capability documented on the Buttons family pages above, including capability with no current Mioframe consumer, is classified below.
...
- **Elevation (resolved):** local shadow-color overrides reach the final `box-shadow` through a pre-existing shared foundation contract in `src/shared/lib/md/tokens.css`: `--md-sys-elevation-level0..5` are declared on `*, ::before, ::after` so any element's local `--md-private-elevation-shadow-color` override is picked up. This mechanism predates this Button work and is already the exact pattern `MDCard`, `MDFab`, `MDExtendedFab`, and `MDSwitch` rely on for their own local shadow-color overrides. `MDButton` follows the same established contract rather than introducing a new one; representative cross-family safety is evidenced by those four pre-existing consumers continuing to pass their own tests/visual coverage unchanged. Narrowing this shared mechanism to a non-universal owner remains a legitimate future improvement, but it is a cross-family foundation decision (see `material-foundation`), not a Button-scoped one.
...
- **Documentation consistency (resolved):** `guidelines.md` has now been inspected directly through the `material3` MCP cache in this pass; the snapshot above is the single current source of record.
