...
description: Use only when the Material orchestrator delegates isolated canonical-target research before current implementation is inspected.
...
disallowedTools: Write, Edit, NotebookEdit, Bash, Agent, Task, mcp__material3__refresh_material_docs
skills:
...
Execute the preloaded `material-canonical-target` role for the delegated scope. Return its required result only. Do not add repository-specific workflow policy here.
