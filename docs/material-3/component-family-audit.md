...
- exact custom-override routing and default no-override role resolution are separate table-driven checks. The default checks cover resting container/icon/outline, hover/focus/pressed icon and state-layer color/opacity (including the rendered state-layer background), outlined interaction outline behavior, and disabled precedence. The custom toggle matrix independently asserts resting icon, hovered icon, state-layer color, opacity, and rendered tint for all four styles × selected/unselected branches, so swapping those routes fails;
