...
- a compact table-driven selected/unselected toggle-routing fixture and test cover all four styles that publish distinct selected/unselected color tokens (`elevated`, `filled`, `tonal`, `outlined`), independently verifying container, label, icon, state-layer color, state-layer opacity, and outline (`outlined` only); container and outline are read at rest, while label/icon/state-layer color/opacity and rendered tint are independently verified for hover, focus, and pressed with distinct fixture literals per route and branch; `text` is verified separately for controlled `aria-pressed`, selected shape, and the unchanged normal text-button resting/interaction label and icon routes because `md.comp.button.text` publishes no selected/unselected color tokens;
