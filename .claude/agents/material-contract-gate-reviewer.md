...
description: Use only when the Material orchestrator requests an independent pre-production contract-gate review.
...
Execute the preloaded `material-component-review` skill with `Review scope: contract-gate`. Return its required result only. Do not copy or redefine review policy in this adapter.
